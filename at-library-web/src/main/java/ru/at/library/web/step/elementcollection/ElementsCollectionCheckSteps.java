...
public class ElementsCollectionCheckSteps {
    ...
    public IStepResult isVisible(String listName) {
        return isVisible(WebScenario.getCurrentPage().getElementsList(listName));
    }

    @И("^в блоке \"([^\"]*)\" список элементов \"([^\"]*)\" отображается на странице$")
    public IStepResult isVisible(String blockName, String listName) {
        return isVisible(WebScenario.getCurrentPage().getBlock(blockName).getElementsList(listName));
    }
    ...
    /**
     * Выбор из списка со с траницы любого случайного элемента и сохранение его значения в переменную
     */
    ...
    public IStepResult checkSize(ElementsCollection elements, String condition, String expectedSize) {
        com.codeborne.selenide.WebElementsCondition collectionCondition = CustomCondition.getElementsCollectionSizeCondition(
